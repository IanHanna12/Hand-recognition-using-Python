...
while cap.isOpened():
    ...
    if hands:
        ...
        # Send rotate command immediately when thumbs up gesture is detected
        if thumbs_up_count == 2:
            ws.send("a")  # Rotate the piece
            print("Rotate command sent")
            thumbs_up_count = 0  # Reset the thumbs up count to avoid repeated commands

        new_gesture = None
        if left_hand_fist and right_hand_open and (current_gesture != "left" or current_time - last_gesture_time > left_right_duration):
            ws.send("l")  # Move the piece left
            print("Move left command sent")
            current_gesture = "left"
            last_gesture_time = current_time
        elif right_hand_fist and left_hand_open and (current_gesture != "right" or current_time - last_gesture_time > left_right_duration):
            ws.send("r")  # Move the piece right
            print("Move right command sent")
            current_gesture = "right"
            last_gesture_time = current_time
        elif left_hand_open and right_hand_open and (current_gesture != "down" or current_time - last_gesture_time > left_right_duration):
            ws.send("d")  # Drop the piece faster
